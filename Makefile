...
KEYMAP_RAW_YAML ?= $(KEYMAP_DOCS_DIR)/efogtech_trackball_0.raw.yaml
...
keymap-yaml:
	...
	uvx --from keymap-drawer keymap parse -z $(KEYMAP_FILE) > $(KEYMAP_RAW_YAML)
	uv run scripts/pretty_keymap_yaml.py $(KEYMAP_RAW_YAML) > $(KEYMAP_YAML)
